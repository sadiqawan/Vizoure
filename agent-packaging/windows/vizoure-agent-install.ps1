...
if ($null -ne $agentExe) {
    ...
    # Create Vizoure Agent service using New-Service (more reliable than sc.exe)
    $exePath = $agentExe.FullName
    New-Service -Name $ServiceName `
        -DisplayName $ServiceName `
        -Description "Vizoure NMS monitoring agent. Collects system metrics and reports to Vizoure NMS server." `
        -BinaryPathName "`"$exePath`" --config `"$ConfigFile`"" `
